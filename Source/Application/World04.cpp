...
namespace nc
{
	...
	void World04::Update(float dt)
	{
		...
		{
			ImGui::DragFloat("Range", &m_lights[m_selected].range, 0.1f, 0.1f, 60);
		}
		//ImGui::ColorEdit3("Ambient Color", glm::value_ptr(m_ambientLight));
		ImGui::End();
		...
		for (int i = 0; i < 3; i++)
		{
			...
			material->GetProgram()->SetUniform(name + ".position", normalize(m_lights[i].position));
