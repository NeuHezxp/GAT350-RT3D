...
vec3 ads(in vec3 position, in vec3 normal)
{
	...
	//ATTENUATION
	float attenuation = 1;
if (light.type != DIRECITONAL)
	{
		float distnacesqr = dot(light.position - position, light.position - position);
		float rangeSqr = light.range * light.range;
		attenuation = max(0, 1 - pow((distnacesqr / rangeSqr), 2.0));
	...
	if (light.type == SPOT)
	{
		...
		spotIntensity = smoothstep(light.outerAngle + .001,light.innerAngle,angle);
	}
